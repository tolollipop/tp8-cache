...
namespace tolollipop\cache\cache\driver;

use FilesystemIterator;
use think\App;
use tolollipop\cache\cache\Driver;
...
class File extends Driver
{
    ...
    {
        $raw = $this->getRaw($name);
        if (empty($raw) || empty($raw['expire'])) {
            return 0;
        }
        return $raw['filemtime'] + $raw['expire'] - time();
    }

    /**
     * 更新缓存内容
     * 该方法区别于set方法，set方法不支持更新内容同时保留缓存剩余有效期
     * @param string $name 缓存标识名
     * @param mixed $newVal 新的内容
     * @param bool $isMerge 是否与更新前的数据合并(仅数组类型有效)
     * @return bool
     */
    public function update(string $name, $newVal, bool $isMerge = true): bool
    {
        $raw = $this->getRaw($name);
        if (empty($raw)) {
        ...
        return $this->set($name, $newVal, $this->ttl($name));
    }
}
